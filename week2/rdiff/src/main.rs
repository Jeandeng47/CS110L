...
fn lcs(seq1: &Vec<String>, seq2: &Vec<String>) -> Grid {
    ...
    for i in 0..m {
        for j in 0..n {
            if seq1[i] == seq2[j] {
                let diagonal = grid.get(i, j).unwrap();
                grid.set(i + 1, j + 1, diagonal + 1).unwrap();
            } else {
                let left = grid.get(i + 1, j).unwrap();
                let top = grid.get(i, j + 1).unwrap();
                grid.set(i + 1, j + 1, usize::max(left, top)).unwrap();
            }
        }
    }
    grid
...
fn main() {
    ...
    let lines1 = read_file_lines(filename1).expect("Could not read from the file 1");
    let lines2 = read_file_lines(filename2).expect("Could not read from the file 2");
