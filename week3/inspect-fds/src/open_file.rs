...
impl OpenFile {
    ...
    pub fn from_fd(pid: usize, fd: usize) -> Option<OpenFile> {
        ...
        // read the destination path that symbolic link points to
        let path = fs::read_link(&fd_path).ok()?;
        let file_string = path.to_str()?; // pathbuffer -> string
        let file_name = OpenFile::path_to_name(&file_string); // string -> readable string

        // read the contents of fd info
        let fd_info_path = format!("/proc/{}/fdinfo/{}", pid, fd);
        let fd_info_content = fs::read_to_string(&fd_info_path).ok()?;

        // extract cursor and flags
        let cursor = OpenFile::parse_cursor(&fd_info_content)?;
        let access_mode = OpenFile::parse_access_mode(&fd_info_content)?;

        Some(OpenFile::new(file_name, cursor, access_mode)) // return new OpenFile
    }
